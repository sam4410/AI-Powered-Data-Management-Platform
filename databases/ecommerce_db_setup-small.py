...
class EnhancedECommerceDataSetup:
    def __init__(self):
        self.db_path = "ecommerce_db-small.db"
        self.logger = logger
    ...
    def _populate_departments(self, cursor):
        departments = [
            ('Sales', 5000, 5, 'New York', 'SALES001'),
            ('Marketing', 3000, 3, 'San Francisco', 'MKT001'),
            ('Engineering', 8000, 10, 'Seattle', 'ENG001'),
            ('HR', 2000, 2, 'Chicago', 'HR001'),
            ('Finance', 4000, 2, 'New York', 'FIN001'),
            ('Operations', 6000, 10, 'Dallas', 'OPS001'),
            ('Customer Service', 2500, 8, 'Austin', 'CS001'),
            ('Data Analytics', 3500, 5, 'Boston', 'DA001')
        ]
        cursor.executemany("""
    ...
    def _populate_warehouses(self, cursor):
        warehouses = [
            ('East Coast Hub', 'New York, NY', 500, 0.75, 1500),
            ('West Coast Hub', 'Los Angeles, CA', 600, 0.80, 1800),
            ('Central Hub', 'Chicago, IL', 450, 0.70, 1400),
            ('South Hub', 'Atlanta, GA', 400, 0.65, 1300),
            ('Northwest Hub', 'Seattle, WA', 350, 0.60, 1200)
        ]
        cursor.executemany("""
    ...
    def _populate_employees(self, cursor):
        ...
        for i in range(46):
            ...
            employee = (
                ...
                salary,
                random.randint(1, 46) if random.random() > 0.3 else None,
                random.choice([True, False]),
                round(random.uniform(1.0, 5.0), 2),
                round(random.uniform(5000, 30000), 2),
                round(random.uniform(4000, 25000), 2),
    ...
    def _populate_orders(self, cursor):
        ...
        orders = []
        for i in range(1, 801):  # 25000 orders (1.67 orders per customer on average)
            ...
            order = (
                i,  # order_id
                random.randint(1, 500),  # customer_id (updated to match 15000 customers)
                order_date,
                random.choice(statuses),
                total_amount,
                discount_amount,
    ...
    def _populate_sales(self, cursor):
        ...
        for order_id in range(1, 801):  # Updated to match 25000 orders
    ...
    def _populate_customer_interactions(self, cursor):
        ...
        interactions = []
        for i in range(1, 2501):  # 75000 interactions (5 per customer on average)
            interaction = (
                i,
                random.randint(1, 500),  # customer_id (updated to match 15000 customers)
    ...
    def _populate_customer_support(self, cursor):
        ...
        tickets = []
        for i in range(1, 401):  # 12000 tickets (0.8 tickets per customer)
    ...
    def _populate_loyalty_program(self, cursor):
        ...
        loyalty_records = []
        for i in range(1, 501):  # 15000 loyalty records (1:1 mapping with customers)
    ...
    def _populate_returns(self, cursor):
        ...
        returns = []
        for i in range(1, 101):  # 3000 returns (12% return rate)
            order_id = random.randint(1, 800)  # Updated to match 25000 orders
    ...
    def _populate_payments(self, cursor):
        ...
        payments = []
        for i in range(1, 801):  # 25000 payments (one payment per order)
            payment = (
                i,
    ...
    def _populate_shipping(self, cursor):
        ...
        shipments = []
        for i in range(1, 721):  # 22500 shipments (90% of orders)
    ...
    def _populate_financial_transactions(self, cursor):
        ...
        transactions = []
        for i in range(1, 1001):  # 40000 transactions (increased for more financial data)
            transaction = (
                i,
                random.choice(transaction_types),
                round(random.uniform(-1000.0, 10000.0), 2),  # amount (negative for expenses)
                'USD',
                fake.date_between(start_date='-2y', end_date='today'),
    ...
    def _populate_product_reviews(self, cursor):
        ...
        reviews = []
        for i in range(1, 1001):  # 30000 reviews (2 per customer on average)
            review = (
                i,
                random.randint(1, 200),  # product_id
                random.randint(1, 500),  # customer_id (updated to match 15000 customers)
    ...
    def _populate_abandoned_carts(self, cursor):
        ...
        carts = []
        for i in range(1, 251):  # 7500 abandoned carts (0.5 per customer)
            cart = (
                i,
                random.randint(1, 500),  # customer_id (updated to match 15000 customers)
                fake.uuid4(),  # session_id
    ...
    def _populate_email_campaigns(self, cursor):
        ...
        campaigns = []
        for i in range(1, 3001):  # 90000 email campaigns (6 per customer on average)
            campaign = (
                i,
    ...
    def _populate_referrals(self, cursor):
        ...
        referrals = []
        for i in range(1, 76):  # 2250 referrals (15% of customers referring)
            referral = (
                i,
                random.randint(1, 500),  # referrer_id (updated to match 15000 customers)
                random.randint(1, 500),  # referee_id (updated to match 15000 customers)
                fake.date_between(start_date='-1y', end_date='today'),
